...
const hiddenMultiplier = 1.2; // Adjust this to control the strength of preferences

// Bonus weight for the final question (animal choice)
const bonusWeight = 1; // Adjust this to control the impact of the final question

// Predefined personality traits for all 16 MBTI types (unchanged)
const personalityTraits = {
  "INFP": {
    ...
    famousPersonalities: ["J.R.R. Tolkien", "Princess Diana", "Audrey Hepburn"],
  ...
  "INFJ": {
    ...
    famousPersonalities: ["Martin Luther King Jr.", "Nelson Mandela", "Mother Teresa"],
  ...
  "INTP": {
    ...
    famousPersonalities: ["Albert Einstein", "Isaac Newton", "Marie Curie"],
  ...
  "INTJ": {
    ...
    famousPersonalities: ["Elon Musk", "Nikola Tesla", "Ayn Rand"],
  ...
  "ISFP": {
    ...
    famousPersonalities: ["Bob Dylan", "Marilyn Monroe", "Michael Jackson"],
  ...
  "ISTP": {
    ...
    famousPersonalities: ["Clint Eastwood", "Bear Grylls", "Amelia Earhart"],
  ...
  "ISTJ": {
    ...
    famousPersonalities: ["George Washington", "Angela Merkel", "Jeff Bezos"],
  ...
  "ENFP": {
    ...
    famousPersonalities: ["Robin Williams", "Walt Disney", "Ellen DeGeneres"],
  ...
  "ENFJ": {
    ...
    famousPersonalities: ["Oprah Winfrey", "Barack Obama", "Maya Angelou"],
  ...
  "ENTP": {
    ...
    famousPersonalities: ["Thomas Edison", "Mark Twain", "Socrates"],
  ...
  "ENTJ": {
    ...
    famousPersonalities: ["Steve Jobs", "Margaret Thatcher", "Napoleon Bonaparte"],
  ...
  "ESFP": {
    ...
    famousPersonalities: ["Elvis Presley", "Marilyn Monroe", "Will Smith"],
  ...
  "ESFJ": {
    ...
    famousPersonalities: ["Taylor Swift", "Bill Clinton", "Jennifer Garner"],
  ...
  "ESTP": {
    ...
    famousPersonalities: ["Ernest Hemingway", "Madonna", "Eddie Murphy"],
  ...
  "ESTJ": {
    ...
    famousPersonalities: ["Henry Ford", "Michelle Obama", "Sonia Sotomayor"],
  ...
  // ... and the rest of the 16 MBTI types ..
...
function calculateMBTI(answers) {
    ...
    // Calculate scores for the first 15 questions (5 sections * 3 questions)
    for (let index = 0; index < Math.min(15, answers.length); index++) {
        const section = sections[Math.floor(index / questionsPerSection)];
        const baseScore = answers[index] === 0 ? -1 : 1; // Base score: -1 for 0, 1 for 1
        const mappedScore = baseScore * psychologicalWeights[section] * hiddenMultiplier; // Apply hiddenMultiplier
    ...
    // Handle the final question (animal choice) separately
    if (answers.length > 15) {
        const finalAnswer = answers[15];
        console.log('Final Answer:', finalAnswer);
        switch (finalAnswer) {
            case 0: // Wolf -> Introvert
                scores.mind += bonusWeight;
                break;
            case 1: // Dolphin -> Extrovert
                scores.mind -= bonusWeight;
                break;
            case 2: // Owl -> Intuitive
                scores.energy += bonusWeight;
                break;
            case 3: // Tiger -> Judging
    ...
    const maxPossibleScore = questionsPerSection * maxScorePerQuestion * hiddenMultiplier; // Adjust max score for multiplier
...
const storeResult = async (req, res) => {
    try {
        ...
        if (!personalityDetails) {
            ...
            personalityDetails = new Personality(traits);
            await personalityDetails.save();
        }

        const topProjects = await findBestMatches(userId, type); // Pass the calculated type
